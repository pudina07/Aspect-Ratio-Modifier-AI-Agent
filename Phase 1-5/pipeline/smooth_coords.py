...
def generate_mock_final_coords(
    ...
    coords_916["meta"] = {
        "crop_width": crop_w_916,
        "crop_height": crop_h_916,
        "source_width": src_w,
        "source_height": src_h,
        "fps": fps,
    ...
    coords_11["meta"] = {
        "crop_width": crop_w_11,
        "crop_height": crop_h_11,
        "source_width": src_w,
        "source_height": src_h,
        "fps": fps,
        "frame_count": total_frames
...
def main():
    ...
    try:
        ...
        crop_w_916 = coords_916.get("target_width", coords_916.get("meta", {}).get("crop_width"))
        crop_h_916 = coords_916.get("target_height", coords_916.get("meta", {}).get("crop_height"))
        crop_w_11 = coords_11.get("target_width", coords_11.get("meta", {}).get("crop_width"))
        crop_h_11 = coords_11.get("target_height", coords_11.get("meta", {}).get("crop_height"))
        print(f"[{STAGE_NAME}] successfully wrote {args.out_916} "
              f"({crop_w_916}x{crop_h_916}) and "
              f"{args.out_11} ({crop_w_11}x{crop_h_11}), "
