...
def run_contract_boundary_tests():
    ...
    invalid_transcripts = [
        ...
        {"words": [{"word": "hi", "start": 3.0, "end": 2.0}]},
    ]
    ...
    invalid_coords = [
        {"fps": 30.0, "width": 1920},
        {"fps": 30.0, "width": 1920, "height": 1080, "total_frames": 1, "frames": [{}]},
    ]
    ...
    invalid_text = [
        {"fps": 30.0, "width": 1920, "height": 1080, "regions": [{"t_start": 0, "t_end": 1, "box": [1, 2, 3]}]},
        {"regions": "invalid"},
    ]
...
def run_parallel_stress_test():
    print("\n--- 4. Multi-Workspace Parallel Stress Test (6 Isolated Runs) ---")
...
def run_physical_video_frame_integrity_test():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        report = run_pipeline(data_dir=ws, mock=True)
        assert report.ok is True

        out_916 = ws / "output_916.mp4"
        out_11 = ws / "output_11.mp4"

        for out_file, expected_w, expected_h in [(out_916, 1080, 1920), (out_11, 1080, 1080)]:
