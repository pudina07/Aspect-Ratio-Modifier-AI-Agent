...
def run_all_stress_tests() -> Dict[str, Any]:
    ...
    # 1. Transcription mock & live test
    print("\n--- 1. Multi-File Transcription Benchmarks ---")
    mock_res = run_transcribe(Path("nonexistent.mp4"), mock=True)
    validate_transcript(mock_res)
    assert len(mock_res["words"]) > 0
    print("  [PASS] Mock transcription test verified.")

    test_files = [
        ("speech.wav", PROJECT_ROOT / "assets" / "speech.wav"),
        ("test_clip_16_9.mp4", PROJECT_ROOT / "assets" / "test_clip_16_9.mp4"),
        ("Test_Video.mp4", PROJECT_ROOT / "Test_Video.mp4"),
    ]
    ...
    for label, path in test_files:
        ...
        try:
            ...
            print(f"  [SKIP] {label} model execution: {e}")
