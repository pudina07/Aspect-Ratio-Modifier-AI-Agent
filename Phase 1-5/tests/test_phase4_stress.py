...
def test_one_euro_filter_invariants():
    ...
    # 1. Steady-state noise filtering
    np.random.seed(42)
    ...
    # 2. Dynamic beta responsiveness on step input
    ...
    res_same_t = f_guard(t=1.0, x=55.0)
    res_back_t = f_guard(t=0.5, x=60.0)
...
def test_smoothstep_easing_curve():
    ...
    assert _ease_in_out(-0.5) == 0.0
    assert _ease_in_out(1.5) == 1.0
