"""
config.py — Phase 1 & 5: Pipeline Architecture & Configuration
...
# Prioritize project root models if populated, fallback to local models dir
if (PROJECT_ROOT / "models").exists() and any((PROJECT_ROOT / "models").iterdir()):
    MODELS_DIR = PROJECT_ROOT / "models"
else:
    MODELS_DIR = BASE_DIR / "models"

# Safe zones config path
if (BASE_DIR / "safe_zones.json").exists():
    SAFE_ZONES_PATH = BASE_DIR / "safe_zones.json"
else:
    ...
    if sz_file.exists():
        try:
            with open(sz_file, "r", encoding="utf-8") as f:
                zones = json.load(f)
                return {k: v for k, v in zones.items() if not k.startswith("_")}
        except Exception:
            pass
