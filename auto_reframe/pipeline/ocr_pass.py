"""
pipeline/ocr_pass.py — Phase 1: Architecture & Contract Stub
...
Supports --mock flag to generate valid schema-compliant mock text regions for testing Phase 1.
"""
import argparse
import sys
...
def generate_mock_text_regions(video_path: Path) -> dict:
    ...
    regions = [
        # Top-right critical caption during segment 3 (6.8s - 10.37s)
        TextRegion(
            t_start=6.8,
            t_end=10.37,
        ...
        # Bottom-left CTA caption during segment 3 (6.8s - 10.37s)
        TextRegion(
            t_start=6.8,
            t_end=10.37,
...
def run(video_path: Path, mock: bool = False) -> dict:
    ...
    raise NotImplementedError(
        "Phase 3, Step 6: EasyOCR every 8th frame -> protected text regions. (Use --mock for architecture testing)"
    )
