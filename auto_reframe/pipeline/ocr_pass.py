...
def run(
    ...
    active: List[Dict[str, Any]] = []  # {"box": (x,y,w,h), "t_start": t, "t_end": t, "text": str, "conf": float}
    ...
    try:
        while True:
            ...
            if frame_idx % sample_rate == 0:
                ...
                # Transition unmatched active regions to finished
                still_active = []
                for i, region in enumerate(active):
                    if i in matched:
    ...
    # Flush active regions at video end
    finished_regions.extend(active)

    # Convert to TextRegion contracts
    ...
    # Add legacy compatibility fields and metadata
    result_dict["meta"] = {
        "width": width,
        "height": height,
        "fps": fps,
