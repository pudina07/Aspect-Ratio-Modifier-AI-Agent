"""
pipeline/tracker.py — Phase 1: Architecture & Contract Stub
...
def generate_mock_raw_coords(video_path: Path, focus_timeline: dict) -> dict:
    ...
    for i in range(total_frames):
        t = i / fps
        # Default talking head center at (960, 540)
        ...
        # Between 3.5s and 6.8s (object pointing block)
        if 3.5 <= t <= 6.8:
            focus = "object"
            wrist = [1540.0, 520.0]
            fingertip = [1590.0, 510.0]
            target = [1720.0, 480.0]  # Extrapolated towards chart
...
def run(video_path: Path, focus_timeline: dict, mock: bool = False) -> dict:
    ...
    raise NotImplementedError(
        "Phase 3, Step 4-5: Face detection & pose/hand pointing vector extrapolation. (Use --mock for architecture testing)"
    )
